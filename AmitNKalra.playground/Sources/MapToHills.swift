...
public class MappingToSixthActivity: UIView {
    ...
    public init(scene: UIView) { // Basically the viewDidLoad
        ...
        runner.addTarget(self, action: #selector(pressedRunner), for: .touchUpInside)
    ...
    func setupView() {
        ...
        Timer.scheduledTimer(timeInterval: 0.0, target: self, selector: #selector(loadRunner), userInfo: nil, repeats: false)
    ...
    @objc func loadFirstStep() {
        ...
        firstPath.frame = CGRect(x: 480, y: 260, width: 21, height: 9)
        firstPath.rotate(degrees: -98)
    ...
    @objc func loadSecondStep() {
        ...
        secondPath.frame = CGRect(x: 475, y: 232, width: 21, height: 9)
        secondPath.rotate(degrees: -115)
    ...
    @objc func loadThirdStep() {
        ...
        thirdPath.frame = CGRect(x: 460, y: 210, width: 21, height: 9)
        thirdPath.rotate(degrees: -165)
        self.addSubview(thirdPath)
        
    }
    
    @objc func loadRunner() {
        ...
        runner.frame = CGRect(x: 435, y: 20, width: 200, height: 200)
        self.addSubview(runner)
        
        UIView.animate(withDuration: 5.0, delay: 0.0, usingSpringWithDamping: 1.0, initialSpringVelocity: 1.0, options: UIViewAnimationOptions.curveEaseIn, animations: ({
            
            self.runner.center.x -= 100
            self.runner.center.y += 100
            
        }), completion: nil)
        
    }
    
    @objc func pressedRunner() {
        
        self.removeFromSuperview()
        
        let runningView = RunningObby(scene: self)
        PlaygroundPage.current.liveView = runningView
